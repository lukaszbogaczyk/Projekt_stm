 ...
 */
float BMP180_GetRawTemperature(void) {
    // Wyślij polecenie odczytu temperatury
    BMP180_WriteReg(BMP180_CONTROL_REG, BMP180_CMD_TEMP);
    HAL_Delay(BMP180_DELAY_TEMP);

    // Odczytaj nieprzetworzoną wartość temperatury
    int32_t UT = (BMP180_ReadReg(BMP180_MSB_REG) << 8) | BMP180_ReadReg(BMP180_LSB_REG);

    // Obliczanie temperatury
    long x1 = ((UT - _bmp180_eeprom.BMP180_AC6) * (_bmp180_eeprom.BMP180_AC5 / pow(2, 15)));
    long x2 = ((_bmp180_eeprom.BMP180_MC) * pow(2, 11)) / (x1 + _bmp180_eeprom.BMP180_MD);
    long B5 = x1 + x2;

    // Konwersja na temperaturę w stopniach Celsjusza (dziesiąte części stopnia)
...
float BMP180_GetTemperature(void) {
	float temp = BMP180_GetRawTemperature();
	return temp / 100.0;
}
